import json
from flask import Flask, flash, url_for, make_response
...
def book(competition, club):
    try:
        foundClub = [c for c in clubs
                     if c['name'] == club][0]
        foundCompetition = [c for c in competitions
                            if c['name'] == competition][0]
        if foundClub and foundCompetition and not foundCompetition['over']:
            return render_template(
                'booking.html',
    ...
    if not competition['over']:
        ...
        if total_points_to_deducted <= int(club["points"]):
            if placesRequired <= MAX_PLACES_PER_CLUB:
                ...
                club["points"] = str(
                    int(club["points"]) - total_points_to_deducted
                )
            ...
            message = ("You should not book more than your available points")
    else:
        message = ("The competitionis over, the booking is closed!")
